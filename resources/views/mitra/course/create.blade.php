...
<form action="{{ route('frontend.mitra.course.store') }}" method="post" enctype="multipart/form-data">
    <div class="card">
        <div class="card-header d-flex justify-content-between">
            <h4 class="card-title">{{ __('button.add') }} {{ __('sidebar.course') }}
            </h4>
        ...
        <div class="card-body">
            @csrf
            <x-forms.input name="id" id="id" :isRequired="true" type:"hidden" :label="__('field.course_name')" />
            <x-forms.input name="name" id="name" :isRequired="true" :label="__('field.course_name')" />
            <x-forms.select name="categorie_id" id="categorie_id" :isRequired="true"
                :label="__('field.course_category')">
            ...
            <x-forms.input type="textarea" name="description" id="description" :isRequired="true"
            ...
            <div class="text-end">
                <button type="button" id="btnProcess"
                    class="btn btn-success">{{ __('button.process') }}</button>
            </div>
        </div>
    </div>
    <div class="card">
